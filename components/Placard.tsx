...
export default function Placard({name,year,pos,src}:Props) {
  return (
    <div className={styles.container}>
        ...
        <div className= 'flex flex-col items-center text-center absolute w-[291px] h-1/4 bg-[#292929] rounded-b-[39px] -mb-[1px]'>
        <p className='font-semibold text-3xl text-white m-[25px]'>{name}</p>
        <p className={(year=='1')? styles.year1:styles.year2} >{(year=='1')? "First-Year":"Second-Year"}</p>
        <p className='font-semibold text-lg text-white m-[30px]'>{pos}</p>
        </div>
    </div>
  )
}
